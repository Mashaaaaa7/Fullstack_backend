...
from app.models import User, PDFFile, UserRole, Flashcard, ActionHistory
...
async def upload_pdf(
    ...
    crud.add_action(db, "upload", file.filename, user.user_id, "Файл загружен")
        ...
        "pdfs": [
            {
                "id": p.id,
                "name": p.file_name,
                "file_size": os.path.getsize(p.file_path) if os.path.exists(p.file_path) else 0
            } for p in pdf_files
        ],
        "total": len(pdf_files)
    }
