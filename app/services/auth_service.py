...
class AuthService:
    ...
    def refresh(self, refresh_token: str):
        ...
        db_token = self.token_repo.get_refresh_token(token_id)
        if (
            not db_token
            or db_token.revoked
            or db_token.expires_at < datetime.utcnow()
            or db_token.user_id != user_id
        ):
            raise HTTPException(status_code=401, detail="Refresh token revoked or expired")

        self.token_repo.revoke_refresh_token(token_id)
    ...
    def _create_tokens(self, user):
        ...
        refresh = create_refresh_token(user.user_id, str(token.id))
        return {
            "access_token": access,
